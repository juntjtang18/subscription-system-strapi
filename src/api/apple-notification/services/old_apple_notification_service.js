...
module.exports = ({ strapi }) => ({
  async processNotification(requestBody) {
    ...
    try {
      const decodedPayload = await this.verifyAppleJWS(signedPayload);
      ...
      if (existingLogs && existingLogs.length > 0) {
        logger.warn(`[Apple Webhook SVC] Duplicate notification received (UUID: ${decodedPayload.notificationUUID}). Skipping.`);
        return;
      }

      logEntry = await strapi.entityService.create("api::apple-notification.apple-notification", {
        data: {
      ...
      if (decodedPayload.notificationType === 'TEST') {
        logger.info("[Apple Webhook SVC] TEST notification successfully verified. No transaction data to process.");
        await this.updateLogStatus(logEntry.id, "processed");
        return;
      }

      const signedTransactionInfo = decodedPayload.data.signedTransactionInfo;
      const transactionInfo = await this.verifyAppleJWS(signedTransactionInfo);
      
      // ✨ NEW: Log the decoded transaction info
      logger.info(`[Apple Webhook SVC] Decoded Transaction Info: ${JSON.stringify(transactionInfo, null, 2)}`);
      
      await strapi.entityService.update('api::apple-notification.apple-notification', logEntry.id, {
        data: {
          transactionInfo,
      ...
      const subscriptions = await strapi.entityService.findMany("api::subscription.subscription", {
        filters: { originalTransactionId: transactionInfo.originalTransactionId },
      });

      if (!subscriptions || subscriptions.length === 0) {
        await this.updateLogStatus(logEntry.id, "failed_not_found");
        throw new Error(`No subscription found for originalTransactionId: ${transactionInfo.originalTransactionId}`);
      }
      const subscription = subscriptions[0];

      const updates = this.getSubscriptionUpdates(decodedPayload.notificationType, transactionInfo);
      if (Object.keys(updates).length > 1) {
        await strapi.entityService.update("api::subscription.subscription", subscription.id, { data: updates });
      }

      await this.updateLogStatus(logEntry.id, "processed", subscription.id);
      logger.info(`[Apple Webhook SVC] Successfully processed notification for subscription ID: ${subscription.id}`);
      ...
      logger.error(`[Apple Webhook SVC] Final error in processing: ${error.message}`);
      throw error;
    }
  },
  ...
  getSubscriptionUpdates(notificationType, transactionInfo) {
    ...
    switch (notificationType) {
      ...
      case "REFUND":
        ...
        break;
      default:
        logger.warn(`[Apple Webhook SVC] Unhandled notification type: ${notificationType}.`);
    }
    return updates;
  },
  ...
  async updateLogStatus(logId, status, subscriptionId = null) {
    ...
    if (subscriptionId) { data.subscription = subscriptionId; }
    return strapi.entityService.update('api::apple-notification.apple-notification', logId, { data });
  },
  ...
  async verifyAppleJWS(token) {
    try {
      ...
      logger.error(`[Apple Webhook SVC] JWS verification failed with node-jose: ${error.message}`);
